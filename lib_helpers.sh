...
function get_log_file_name {
    ...
    echo own_script_name_full=$(get_own_script_name "${script_name}" "${bash_source}")
    echo own_script_name_wo_extension=${own_script_name_full%.*}
    echo own_script_name_wo_extension_dashed=$(echo "${own_script_name_wo_extension}" | tr '/' '_' )
    echo "${log_file_name}"
}
